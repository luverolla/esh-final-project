...
typedef enum DEVICE_MODE_WRITE_BYTES {
	...
	EXIT_BOOTLOADER = 0x00, EXIT_RESET = 0x02, ENTER_BOOTLOADER = 0x08
...
typedef enum FIFO_EXTERNAL_INDEX_BYTE {
	...
	SAMPLE_SIZE, READ_INPUT_DATA, READ_SENSOR_DATA, // For external accelerometer
...
typedef enum WRITE_REGISTER_INDEX_BYTE {
	...
	WRITE_MAX30101 = 0x03, WRITE_ACCELEROMETER
...
typedef enum READ_REGISTER_INDEX_BYTE {
	...
	READ_MAX30101 = 0x03, READ_ACCELEROMETER
...
typedef enum GET_AFE_INDEX_BYTE {
	...
	RETRIEVE_AFE_MAX30101 = 0x03, RETRIEVE_AFE_ACCELEROMETER
...
typedef enum DUMP_REGISTER_INDEX_BYTE {
	...
	DUMP_REGISTER_MAX30101 = 0x03, DUMP_REGISTER_ACCELEROMETER
...
typedef enum SENSOR_ENABLE_INDEX_BYTE {
	...
	ENABLE_MAX30101 = 0x03, ENABLE_ACCELEROMETER
...
typedef enum READ_SENSOR_ENABLE_INDEX_BYTE {
	...
	READ_ENABLE_MAX30101 = 0x03, READ_ENABLE_ACCELEROMETER
...
typedef enum ALGO_BPT_WRITE_BYTE {
	...
	BPT_MEDICATION = 0x00, SYSTOLIC_VALUE, DIASTOLIC_VALUE, BPT_CALIB_DATA, //Index + 824 bytes of calibration data
...
typedef enum ALGORITHM_MODE_ENABLE_INDEX_BYTE {
	...
	ENABLE_AGC_ALGO = 0x00, ENABLE_WHRM_ALGO = 0x02
...
typedef enum BOOTLOADER_INFO_INDEX_BYTE {
	...
	BOOTLOADER_VERS = 0x00, PAGE_SIZE
...
typedef enum IDENTITY_INDEX_BYTES {
	...
	READ_MCU_TYPE = 0x00, READ_SENSOR_HUB_VERS = 0x03, READ_ALGO_VERS = 0x07
...
#define BIODATA_ADD(bd, x) {\
	bd.heartrate += x.heartrate;\
	bd.confidence += x.confidence;\
	bd.oxygen += x.oxygen;\
}
...
uint8_t MAX32664_ReadByte(MAX32664_Handle *handle, uint8_t, uint8_t);
